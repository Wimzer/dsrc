...
public class player_utility extends script.base_script
{
    ...
    public static final String VAR_PLANETARY_MINING_RELEASE_SEQUENCES = "planetary_mining.pending_release_sequences";
    public static final String SCRIPTVAR_PLANETARY_MINING_RELEASE_IN_FLIGHT = "planetary_mining.release_in_flight";
    public static final String VAR_PLANETARY_MINING_PENDING_LAUNCH_ITEM = "planetary_mining.pending_launch_item";
    public static final String VAR_PLANETARY_MINING_PENDING_LAUNCH_OPERATION = "planetary_mining.pending_launch_operation";
    public static final String VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE = resource.VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE;
    public static final String VAR_PLANETARY_MINING_DELIVERED_SEQUENCES = "planetary_mining.delivered_sequences";
    public static final String SCRIPTVAR_PLANETARY_MINING_RETURN_PID = "planetary_mining.return.pid";
    public static final String SCRIPTVAR_PLANETARY_MINING_RETURN_JOB = "planetary_mining.return.job";
    public static final String PLANETARY_MINING_STF = "planetary_mining_droid";
    public int OnLogin(obj_id self) throws InterruptedException
    {
        ...
        String pendingLaunchOperation = getStringObjVar(self, VAR_PLANETARY_MINING_PENDING_LAUNCH_OPERATION);
        if (pendingLaunchOperation != null && !pendingLaunchOperation.equals(""))
        {
            planetaryMiningDroidUpdateAccountJob(self, self, getOperationSequence(pendingLaunchOperation), true);
        }
        int activeJobSequence = getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE);
        if (activeJobSequence > 0)
        {
            if (hasDeliveredPlanetaryMiningDroidJob(self, activeJobSequence))
            {
                finishPlanetaryMiningDroidJob(self, activeJobSequence, getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT));
            }
            else
            {
                int startedAt = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_STARTED_AT);
                int duration = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_DURATION);
                float remaining = Math.max(0, startedAt + duration - getCalendarTime());
                if (remaining > 0 && !buff.hasBuff(self, resource.BUFF_PLANETARY_MINING_SURVEY_LICENSE))
                {
                    buff.applyBuff(self, resource.BUFF_PLANETARY_MINING_SURVEY_LICENSE, remaining);
                }
                planetaryMiningDroidUpdateAccountJob(self, self, activeJobSequence, true);
                if (hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT))
                {
                    dictionary retryParams = new dictionary();
                    retryParams.put("jobSequence", activeJobSequence);
                    messageTo(self, "handlePlanetaryMiningDroidReturn", retryParams, 0.0f, true);
                }
                else if (hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_RESOURCE) && getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_SCHEDULED) == 0)
                {
                    dictionary returnParams = new dictionary();
                    returnParams.put("jobSequence", activeJobSequence);
                    if (messageTo(self, "handlePlanetaryMiningDroidReturn", returnParams, remaining, true))
                    {
                        setObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_SCHEDULED, 1);
                    }
                }
            }
        }
    ...
    public int handleDelayedProseMessage(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        prose_package pp = prose.getPackage(sid, actor, actorString, actorStringId, target, targetString, targetStringId, other, otherString, otherStringId, di, df);
        sendSystemMessageProse(self, pp);
        return SCRIPT_CONTINUE;
    }
    ...
    public int handlePlanetaryMiningDroidReturn(obj_id self, dictionary params) throws InterruptedException
    {
        if (params == null)
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "delivery_failed"));
            return SCRIPT_CONTINUE;
        }
        ...
        {
            if (getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE) == jobSequence)
            {
                finishPlanetaryMiningDroidJob(self, jobSequence, getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT));
            }
            return SCRIPT_CONTINUE;
        }
        if (getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE) != jobSequence)
        ...
        if (!hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT))
        {
            ...
            {
                sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "yield_determination_failed"));
                return SCRIPT_CONTINUE;
            }
        ...
        deliverPlanetaryMiningDroidJob(self, jobSequence, resourceType, amount, params);
        return SCRIPT_CONTINUE;
    }
    public void deliverPlanetaryMiningDroidJob(obj_id self, int jobSequence, obj_id resourceType, int amount, dictionary retryParams) throws InterruptedException
    {
        if (amount == 0)
        {
            finishPlanetaryMiningDroidJob(self, jobSequence, amount);
            return;
        }
        ...
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "delivery_failed"));
            messageTo(self, "handlePlanetaryMiningDroidReturn", retryParams, 60.0f, true);
            return;
        }
        obj_id crate = findPlanetaryMiningDroidDeliveryCrate(self, jobSequence);
        if (!isIdValid(crate))
        {
            crate = createObjectInInventoryAllowOverload(crateTemplate, self);
            if (isIdValid(crate))
            {
                setObjVar(crate, resource.VAR_PLANETARY_MINING_DELIVERY_JOB, jobSequence);
            }
        }
        int deliveredAmount = isIdValid(crate) ? getResourceContainerQuantity(crate) : 0;
        obj_id deliveredType = isIdValid(crate) ? getResourceContainerResourceType(crate) : null;
        if (deliveredAmount > 0 && !resourceType.equals(deliveredType))
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "delivery_pending"));
            messageTo(self, "handlePlanetaryMiningDroidReturn", retryParams, 60.0f, true);
            return;
        }
        if (!isIdValid(crate) || deliveredAmount > amount || (deliveredAmount < amount && !addResourceToContainer(crate, resourceType, amount - deliveredAmount, self)) || getResourceContainerQuantity(crate) != amount)
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "delivery_pending"));
            messageTo(self, "handlePlanetaryMiningDroidReturn", retryParams, 60.0f, true);
            return;
    ...
    public obj_id findPlanetaryMiningDroidDeliveryCrate(obj_id self, int jobSequence) throws InterruptedException
    {
        ...
        if (contents != null)
        {
            ...
            {
                if (getIntObjVar(item, resource.VAR_PLANETARY_MINING_DELIVERY_JOB) == jobSequence)
                {
                    return item;
                }
            }
    ...
    public void finishPlanetaryMiningDroidJob(obj_id self, int jobSequence, int amount) throws InterruptedException
    {
        boolean alreadyDelivered = hasDeliveredPlanetaryMiningDroidJob(self, jobSequence);
        enqueuePlanetaryMiningDroidRelease(self, jobSequence);
        if (!alreadyDelivered)
        {
            markPlanetaryMiningDroidJobDelivered(self, jobSequence);
        }
        if (getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE) == jobSequence)
        {
            removeObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB);
            removeObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE);
            ...
            clearPlanetaryMiningDroidReturnSui(self);
        }
        if (!alreadyDelivered)
        {
            sendSystemMessageProse(self, prose.getPackage(new string_id(PLANETARY_MINING_STF, "resources_returned"), amount));
        }
    }
    public int returnPlanetaryMiningDroid(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        if (params != null && !params.trim().equals(""))
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "return_command_no_parameters"));
            return SCRIPT_OVERRIDE;
        }
        int jobSequence = getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE);
        if (jobSequence < 1)
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "no_active_expedition"));
            return SCRIPT_OVERRIDE;
        }
        ...
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "invalid_expedition"));
            return SCRIPT_OVERRIDE;
        }
        if (hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT))
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "already_returning"));
            return SCRIPT_OVERRIDE;
        }
        obj_id resourceType = getObjIdObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_RESOURCE);
        int fullAmount = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_AMOUNT);
        int projectedAmount = getPlanetaryMiningDroidProjectedAmount(self);
        if (!isIdValid(resourceType) || fullAmount < 1 || projectedAmount < 0)
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "invalid_expedition_natural_completion"));
            return SCRIPT_OVERRIDE;
        }
        clearPlanetaryMiningDroidReturnSui(self);
        int elapsed = Math.max(0, getCalendarTime() - getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_STARTED_AT));
        int duration = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_DURATION);
        String prompt = "Resource: " + utils.localizeSIDString(getResourceName(resourceType)) + "\nProjected Full Yield: " + fullAmount + "\nElapsed time: " + Math.min(elapsed, duration) + " of " + duration + " seconds\nResources Acquired: " + projectedAmount + "\nResources Forfeited: " + (fullAmount - projectedAmount) + "\n\nReturn the droid now?";
        int pid = sui.msgbox(self, self, prompt, sui.YES_NO, "Return Interplanetary Mining Droid", sui.MSG_QUESTION, "handleReturnPlanetaryMiningDroidConfirmation");
        if (pid >= 0)
        {
            utils.setScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID, pid);
            utils.setScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_JOB, jobSequence);
        }
        return SCRIPT_OVERRIDE;
    }
    public int handleReturnPlanetaryMiningDroidConfirmation(obj_id self, dictionary params) throws InterruptedException
    {
        if (params == null || !utils.hasScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID) || !utils.hasScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_JOB))
        {
            return SCRIPT_CONTINUE;
        }
        int pageId = utils.getIntScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID);
        int jobSequence = utils.getIntScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_JOB);
        if (params.getInt("pageId") != pageId)
        {
            return SCRIPT_CONTINUE;
        }
        clearPlanetaryMiningDroidReturnSui(self);
        ...
        if (getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE) != jobSequence || hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_COMPLETION_AMOUNT))
        {
            sendSystemMessage(self, new string_id(PLANETARY_MINING_STF, "return_unavailable"));
            return SCRIPT_CONTINUE;
        }
        obj_id resourceType = getObjIdObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_RESOURCE);
        int amount = getPlanetaryMiningDroidProjectedAmount(self);
        ...
        dictionary retryParams = new dictionary();
        retryParams.put("jobSequence", jobSequence);
        deliverPlanetaryMiningDroidJob(self, jobSequence, resourceType, amount, retryParams);
        return SCRIPT_CONTINUE;
    }
    public int getPlanetaryMiningDroidProjectedAmount(obj_id self) throws InterruptedException
    {
        int fullAmount = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_AMOUNT);
        int startedAt = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_STARTED_AT);
        int duration = getIntObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_DURATION);
        if (fullAmount < 1 || startedAt < 1 || duration < 1)
        {
            return -1;
        }
    ...
    {
        if (utils.hasScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID))
        {
            forceCloseSUIPage(utils.getIntScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID));
            utils.removeScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_PID);
        }
        utils.removeScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RETURN_JOB);
    }
    public int queuePlanetaryMiningDroidRelease(obj_id self, dictionary params) throws InterruptedException
    {
        if (params != null)
        {
            enqueuePlanetaryMiningDroidRelease(self, params.getInt("jobSequence"));
        }
        return SCRIPT_CONTINUE;
    }
    public void enqueuePlanetaryMiningDroidRelease(obj_id self, int jobSequence) throws InterruptedException
    {
        if (jobSequence < 1)
        {
            return;
        }
        int[] pendingReleases = hasObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES) ? getIntArrayObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES) : new int[0];
        ...
        setObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES, updatedReleases);
        startPlanetaryMiningDroidRelease(self);
    }
    public void startPlanetaryMiningDroidRelease(obj_id self) throws InterruptedException
    {
        if (!hasObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES) || utils.hasScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RELEASE_IN_FLIGHT))
        {
            return;
        }
        int[] pendingReleases = getIntArrayObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES);
        if (pendingReleases == null || pendingReleases.length < 1)
        {
            removeObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES);
            return;
        }
        String operationId = "release:" + pendingReleases[0];
        if (planetaryMiningDroidUpdateAccountJob(self, self, pendingReleases[0], false))
        {
            utils.setScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RELEASE_IN_FLIGHT, operationId);
        }
        else
        {
    ...
    public int handlePlanetaryMiningDroidAccountJobResponse(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        String pendingLaunchOperation = getStringObjVar(self, VAR_PLANETARY_MINING_PENDING_LAUNCH_OPERATION);
        if (operationId != null && operationId.equals(pendingLaunchOperation))
        {
            obj_id item = getObjIdObjVar(self, VAR_PLANETARY_MINING_PENDING_LAUNCH_ITEM);
            if (isIdValid(item) && exists(item) && utils.hasScriptVar(item, "planetary_mining.account_reservation_pending"))
            {
                messageTo(item, "handlePlanetaryMiningDroidAccountJobResponse", params, 0.0f, true);
            }
            else
            {
                removeObjVar(self, VAR_PLANETARY_MINING_PENDING_LAUNCH_ITEM);
                removeObjVar(self, VAR_PLANETARY_MINING_PENDING_LAUNCH_OPERATION);
                int operationSequence = getOperationSequence(operationId);
                boolean matchingActiveJob = getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE) == operationSequence && hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_RESOURCE);
                if (params.getBoolean("success") && !matchingActiveJob)
                {
                    enqueuePlanetaryMiningDroidRelease(self, operationSequence);
                }
            }
            return SCRIPT_CONTINUE;
        }
        int activeJobSequence = getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE);
        if (operationId != null && operationId.equals("reserve:" + activeJobSequence))
        {
            if (!params.getBoolean("success"))
            {
                messageTo(self, "retryPlanetaryMiningDroidActiveReservation", null, 60.0f, false);
            }
            return SCRIPT_CONTINUE;
        }
        String releaseInFlight = utils.getStringScriptVar(self, SCRIPTVAR_PLANETARY_MINING_RELEASE_IN_FLIGHT);
        if (operationId == null || !operationId.equals(releaseInFlight))
        {
            return SCRIPT_CONTINUE;
        }
        ...
        if (params.getBoolean("success"))
        {
            ...
            if (pendingReleases != null && pendingReleases.length > 1)
            {
                int[] remainingReleases = new int[pendingReleases.length - 1];
                for (int i = 1; i < pendingReleases.length; ++i)
                {
                    remainingReleases[i - 1] = pendingReleases[i];
                }
                setObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES, remainingReleases);
            }
            else
            {
                removeObjVar(self, VAR_PLANETARY_MINING_RELEASE_SEQUENCES);
    ...
    public int retryPlanetaryMiningDroidActiveReservation(obj_id self, dictionary params) throws InterruptedException
    {
        int activeJobSequence = getIntObjVar(self, VAR_PLANETARY_MINING_ACTIVE_JOB_SEQUENCE);
        if (activeJobSequence > 0 && hasObjVar(self, resource.VAR_PLANETARY_MINING_ACTIVE_JOB_RESOURCE))
        {
            planetaryMiningDroidUpdateAccountJob(self, self, activeJobSequence, true);
        }
        return SCRIPT_CONTINUE;
    }
    public int getOperationSequence(String operationId) throws InterruptedException
    {
        int separator = operationId == null ? -1 : operationId.indexOf(':');
        return separator < 0 ? 0 : utils.stringToInt(operationId.substring(separator + 1));
    }
    public boolean hasDeliveredPlanetaryMiningDroidJob(obj_id self, int jobSequence) throws InterruptedException
    {
        if (!hasObjVar(self, VAR_PLANETARY_MINING_DELIVERED_SEQUENCES))
        {
            return false;
        }
        int[] deliveredSequences = getIntArrayObjVar(self, VAR_PLANETARY_MINING_DELIVERED_SEQUENCES);
        if (deliveredSequences != null)
        {
            for (int deliveredSequence : deliveredSequences)
            {
                if (deliveredSequence == jobSequence)
                {
                    return true;
                }
            }
    ...
    public void markPlanetaryMiningDroidJobDelivered(obj_id self, int jobSequence) throws InterruptedException
    {
        int[] deliveredSequences = hasObjVar(self, VAR_PLANETARY_MINING_DELIVERED_SEQUENCES) ? getIntArrayObjVar(self, VAR_PLANETARY_MINING_DELIVERED_SEQUENCES) : new int[0];
        int retainedCount = Math.min(deliveredSequences.length, 31);
        int[] updatedSequences = new int[retainedCount + 1];
        for (int i = 0; i < retainedCount; ++i)
        {
            updatedSequences[i] = deliveredSequences[deliveredSequences.length - retainedCount + i];
        }
        updatedSequences[retainedCount] = jobSequence;
        setObjVar(self, VAR_PLANETARY_MINING_DELIVERED_SEQUENCES, updatedSequences);
    }
    ...
    public int onGcwPersonalContributionTableDictionaryResponse(obj_id self, dictionary params) throws InterruptedException
    {
    ...
    public int onGcwRegionDefenderTableDictionaryResponse(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        {
            final int existingPid = utils.getIntScriptVar(self, "gcw.gcwRegionDefenderDetailsTablePid");
        ...
        final int[] citiesImperial = getGcwDefenderRegionCitiesImperial(selectedGcwDefenderRegion);
        final int[] citiesRebel = getGcwDefenderRegionCitiesRebel(selectedGcwDefenderRegion);
        final int[] guildsImperial = getGcwDefenderRegionGuildsImperial(selectedGcwDefenderRegion);
        final int[] guildsRebel = getGcwDefenderRegionGuildsRebel(selectedGcwDefenderRegion);
        ...
        final int imperialPct = getGcwImperialScorePercentile(selectedGcwDefenderRegion);
        String details = "GCW Region: " + localize(new string_id("gcw_regions", selectedGcwDefenderRegion)) + " (R:" + (100 - imperialPct) + "%% I:" + imperialPct + "%%)";
        ...
        if (controlledDuration > 0)
        {
            ...
            final int[] convertedTime = player_structure.convertSecondsTime(controlledDuration);
            ...
            details += (" (at least 70%% continuously " + ((imperialPct >= 70) ? "Imperial" : "Rebel") + " controlled for " + durationStr + ")");
        }
        details += "\n";
        details += ("GCW Region Defender Rebel Bonus: " + getGcwDefenderRegionRebelBonus(selectedGcwDefenderRegion) + "%%\n");
        details += ("GCW Region Defender Imperial Bonus: " + getGcwDefenderRegionImperialBonus(selectedGcwDefenderRegion) + "%%");
        final int newPid = sui.tableRowMajor(self, self, sui.OK_ONLY, "@gcw:gcw_region_defender_war_terminal_menu", "onGcwRegionDefenderDetailsTableDictionaryResponse", details, columnHeader, columnHeaderType, rowData, true);
        if (newPid > 0)
        {
    ...
    public int onGcwRegionDefenderDetailsTableDictionaryResponse(obj_id self, dictionary params) throws InterruptedException
    {
        if (utils.hasScriptVar(self, "gcw.gcwRegionDefenderDetailsTablePid"))
        {
            utils.removeScriptVar(self, "gcw.gcwRegionDefenderDetailsTablePid");
    ...
    public int playIconicGCWWrapUpMessage(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (!params.containsKey("string_file") || !params.containsKey("iconicMob") || !params.containsKey("music") || !params.containsKey("kudosText"))
        {
            return SCRIPT_CONTINUE;
        }
        String stringFile = params.getString("string_file");
    ...
    public int handleCityGcwRegionDefenderChoice(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        final int cityId = getCitizenOfCityId(self);
        if (cityId <= 0)
        {
            return SCRIPT_CONTINUE;
        }
        final obj_id cityMayor = cityGetLeader(cityId);
        ...
        final obj_id cityHall = cityGetCityHall(cityId);
        if (!isIdValid(cityHall) || !exists(cityHall))
        {
            return SCRIPT_CONTINUE;
        }
        final int factionId = cityGetFaction(cityId);
        if (factionId == 0)
        {
            return SCRIPT_CONTINUE;
        }
        ...
        selectedGcwDefenderRegion = selectedGcwDefenderRegion.substring(indexSeparator + 1);
        final String currentGcwDefenderRegion = cityGetGcwDefenderRegion(cityId);
        ...
        {
            final int cooldown = previousRegionTimeEndDefend + (isGod(self) ? 10 : 86400) - timeNow;
